...
BRIDGE 206은 20대와 60대를 시작점으로, 그 사이의 모든 세대까지 자연스럽게 잇는 블로그라는 뜻입니다. 같은 글을 서로 다른 세대가 편하게 읽고, 서로의 관점으로 글을 남길 수 있는 방향을 핵심 테마로 잡았습니다.

- 전 화면 글자 크기 설정: `보통`, `크게`, `가장 크게` 3단계로 조절하며, 선택값은 브라우저에 저장됩니다.
- 큰 글씨 모드 전용 레이아웃: 메인, 로그인/회원가입, 내 블로그, 블로그 꾸미기, 글쓰기, 글 상세, 이웃/블로그 찾기 화면에서 글자 크기와 여백, 카드 배치를 함께 조정합니다.
- 메인 소개 배지/기능 설명: BRIDGE 206 소개 영역의 배지를 클릭하면 읽기 편의, 세대 질문, 이웃 연결 기능 설명으로 이동합니다.
- 글쓰기 글감 질문: 글쓰기 화면에서 세대 간 대화를 시작하기 좋은 질문 카드를 제공하고, 클릭하면 제목과 본문에 자연스럽게 반영됩니다.
- 이웃/블로그 찾기 보강: 관심사와 세대를 잇는 블로그 탐색 흐름을 강조하고, 큰 글씨에서도 카드 내용과 버튼이 겹치지 않도록 정리했습니다.
- 히어로 이미지 보정: 메인 히어로의 좌우 가장자리 검은 줄이 보이지 않도록 배경 레이어를 보정했습니다.
